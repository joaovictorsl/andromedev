...
const contentObject = {
  organizations: {
    ...
    description: 'Período de inscrição para organizações que possuem interesse de submeter projetos para o Andromedev',
    link: 'https://docs.google.com/forms/d/e/1FAIpQLSdD8a9ug98Zaat2nyjrC5mMlvKesIsIX8ZqBDIK68smxL6uqw/viewform?usp=sf_link',
  },
  projects: {
  ...
  apprentices: {
    ...
    description: 'Período para alunos interessados aplicarem para serem aprendizes em projetos',
    link: '#',
  }
}
...
const Applications = () => {
  ...
  function getTabActiveStyle(tab) {
    return currTab === tab ? 'bg-purple-100 rounded-t border-indigo-600 text-indigo-600' : 'border-gray-200 hover:text-gray-900'
  }

  useEffect(handleTabChange, [currTab])

  const tabClassName = 'sm:px-6 py-3 w-1/2 sm:w-auto justify-center sm:justify-start border-b-2 title-font font-medium inline-flex items-center leading-none tracking-wider'
  return (
      ...
      <section className="text-dark-gray body-font mt-10">
        <div className="container px-5 py-24 mx-auto flex flex-wrap flex-col">
          <div className="flex mx-auto flex-wrap">
            <button
              className={`${tabClassName} ${getTabActiveStyle('organizations')}`}
          ...
          <div className="flex flex-col text-center w-full">
            {initialTab !== currTab && (
              <p className="lg:w-2/3 mx-auto leading-relaxed text-xs font-bold bg-gray-200 uppercase text-gray-800 py-4 my-8">Prazo de inscrição finalizado</p>
            )}
            <p className="lg:w-2/3 mx-auto leading-relaxed text-xl">{currTabContent.description}</p>
          </div>
        </div>
      </section>
    </Layout>
